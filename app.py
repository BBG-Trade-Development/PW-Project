...
# - Stock in bottles <= 0 -> Renames to "Btls OH"
    ...
    # Ensure Start Date and End Date columns are datetime
    PB_input['Start Date'] = pd.to_datetime(PB_input['Start Date'], errors='coerce')
    PB_input['End Date'] = pd.to_datetime(PB_input['End Date'], errors='coerce')

    # Filter rows where date_entry falls within Start/End Date range
    PB_input = PB_input[
        (PB_input['Start Date'] <= date_entry) &
        (PB_input['End Date'] >= date_entry)
    ]
